...
%	(Head ~= Var = coin~=false ->trace;true),
	...
	query_propagation(true) ->
		bb_get(q,QQin),simplify_queryAND(Head,Var,QQin,QQout),flatten(QQout,QQout1),bb_put(q,QQout1),writeln(q(QQout1))
	;
		true
	),
	Var=Val.
...
simplify_queryAND(_,_,[],[]) :-!.
simplify_queryAND(Head,Var,[H|T],Out) :-
	(
		H=(A,B) ->
			simplify_queryOR(Head,Var,(A,B),Hout)
		;
		(
			H= Head~=V ->
				(V\=Var-> Out=false;Hout=true)
				;
				(
					H=(Num is Num2*Num3)->
					(invertformula(H),Hout=true)
					;		
					Hout=H
				)
		)
	...
	\+ground(Out)->
	(
		simplify_queryAND(Head,Var,T,QQoutT),
		(
		Hout==true ->
			Out=QQoutT
		;
...
simplify_queryOR(Head,Var,(A,B),Out) :-
	simplify_queryAND(Head,Var,A,OutA),
	(B=(_,_)->
		simplify_queryOR(Head,Var,B,OutB)
	;
		simplify_queryAND(Head,Var,B,OutB)
	),
	(
	(OutA==false) ->
		Out=OutB
...
getbody(A,[],[]):-!.
getbody(H~=V,[(HH,DD,BB)|T],[BBList2|BBT]) :-
	test_to_list(BB,BBList),
...
%	writeln((H~=V,HH,DD,BB)),
	expandquery2(BBList,BBList2),
	test_to_list(BBList3,BBList2),
	getbody(H~=V,T,BBT),!.

expandquery2([],[]) :-!.
expandquery2([A|T],NewE) :-
	expandquery2(T,ExpT),
	(A = H ~= V ->
	(
		findall((H,Distribution,Body),user:distributionalclause(H,Distribution,Body,_),L),
		(L=[(HH,DD,BB)] ->
		(
			test_to_list(BB,BBList),
			HH=H,
	...
	;
	(
		findall((A,Body),user:hardclause(A,Body,_),L),writeln(L),
		(L=[(HH,BB)] ->
		(
			test_to_list(BB,BBList),
			HH=A,%trace,
...
%	assert_evidence(Query,query),
	...
	(Expand==true ->
	(
		expandquery2(ListQuery,InitialQuery)
	)
	;
	(
...
%	user:proposals,
	...
	(Expand==true ->
	(
		expandquery2(PosEvidence,InitialPosEvidence)
	)
	;
	(
...
%	remove_duplicates(TrueList4, TrueList5),
	...
	(query_proof(Key,ListClean3)->true;halt).	
...
%	trace,
	...
	(query_proof(Key,(NextState,Reward))->true;halt),	
...
%	writeln(nextstate(Key,NextState,ListNextState)),
	...
	flatten(TrueNext,TrueNextList),
	remove_duplicates(TrueNextList, TrueNextList2),
	writeln(nextstate(TrueNextList2)),
	partialproof_det(Key,Reward,ListReward,TrueNextList2), % regression reward
	writeln(partialproof_det(Key,Reward,ListReward,TrueNextList2)),
	writeln(reward(ListReward)),
	test_to_list(TupleNextproof,TrueNextList2),
	writeln(nextproof(TupleNextproof,ListNextState)),
	cleanformula((ListReward,TupleNextproof),ListClean),

	partialproof_true(Key,ListClean,_,True2),
	flatten(True2,TrueList2),
	remove_duplicates(TrueList2, TrueList3),
	test_to_list(TrueList4,TrueList3),
	cleanformula2(TrueList4,ListClean3),
	
	recorded(Key,action(Action),_),
	writeln(action(Action)),
	writeln(regressionproof3(Key,ListClean3)),nl.*/
...
%	writeln(beforefinal(TrueList4,ListClean2)),
	...
	writeln(regressionproof3(ListClean3)).*/
...
query_proof2(Key,A) :-
	...
	(
		...
		(
			A=findall_forward(X,Y,Z)
			->
				findall(X,distributionalclause:query_proof2(Key,Y),Z);
				user:A
		)
		;
		(
...
/*
% ------------------------------------------------------------------------------------------
% regression trying to exploit first the facts in Truelist, then the interepretation
...
partialproof_det(Key,A ~= V,ProofA,TrueList) :-
	...
	((tabling_partialproof_det2(Key,A,D,ProofA,TrueList),likelihood_weighting(V,D,WVal),WVal>0)->true;ProofA= (A ~= V) ).
...
tabling_partialproof_det2(Key,Head,Distribution,L,TrueList) :-
	user:distributionalclause(Head,Distribution,Body,_),
	distributionalclause:query_proof(Key,Body),
...
%	trace,
%	dcpf:printkeyp(Key),nl,
	findall(Reg,(user:hardclause(Head,Body,_),writeln((Head,Body)),
			distributionalclause:partialproof_negation(Key,\+ Body,Reg)),L),
	L\=[],
	writeln(listneg(L)),test_to_list(Proof,L),!.
%	ListB\=[],
