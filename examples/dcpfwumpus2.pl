...
maze(X,Y):t ~ finite([0.8:free,0.2:pit]) := ground(X),ground(Y),X>=0,Y>=0,(X,Y)\=(0,0).
maze(X,Y):t ~ finite([1:wall]) := ground(X),ground(Y),X<0.
maze(X,Y):t ~ finite([1:wall]) := ground(X),ground(Y),Y<0.
...
kl(Particles,KL,Diff) :-
	...
	time((step_particle([action(null)],[observation(breeze) ~=false],Particles),
...
kl_aux(Particles,KL,Diff) :-
	init_particle(Particles),
	time((step_particle([action(null)],[observation(breeze) ~=false],Particles),
	step_particle_aux([action(up)],[observation(breeze) ~=true],Particles),
	step_particle_aux([action(down)],[observation(breeze) ~=false],Particles),
	step_particle_aux([action(right)],[observation(breeze) ~=true],Particles))),
	eval_query_particle((current(maze(0,2))~=pit,current(maze(1,1))~=pit,current(maze(2,0))~=pit),Particles,W1),
	eval_query_particle((current(maze(0,2))~=pit,current(maze(1,1))~=pit,current(maze(2,0))~=free),Particles,W2),
	eval_query_particle((current(maze(0,2))~=pit,current(maze(1,1))~=free,current(maze(2,0))~=pit),Particles,W3),
	eval_query_particle((current(maze(0,2))~=free,current(maze(1,1))~=pit,current(maze(2,0))~=pit),Particles,W4),
...
meankl(Particles,N,MKL) :-
	findall(KL,(between(1,N,I),kl(Particles,KL,Diff)),L),sum_list(L,S),MKL is S/N.
