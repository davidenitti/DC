...
:- set_options(default).
:- set_inference(false).
% this represent the belief at time t for all variables maze(X,Y) not (yet) sampled, this means that this DC clause is used only for variables not in the particle
% we want to keep the query result of maze(X,Y):t not grounded (e.g. DC clause not range-restricted).
% if we make the DC clause range-restricted e.g. adding between(-2,2,X),between(-2,2,Y), this would ground the query answers of maze(X,Y):t, sampling all the maze, we want to avoid that.
maze(X,Y):t ~ finite([0.0:pit,0.6:free,0.4:wall])  := between(-2,2,X),between(-2,2,Y),(X,Y)\=(0,0).
